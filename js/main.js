...
function updateTheme(scrollPos) {
  ...
  // ширина и пик "нырка в чёрное" (см. servicesExitVeilEl ниже). Раньше
  // была уже EXIT_FADE_PX (360) — весь эффект укладывался в самый хвост
  // угасания картинки и читался как резкий "щелчок в черноту" перед
  // самым концом, а не как заранее заметный, неторопливый переход.
  // Сейчас окно куда шире самого угасания картинки (900 в каждую
  // сторону от границы, то есть чернота начинает еле заметно
  // проступать ещё вполне заметную часть чтения последней услуги, пока
  // сама картинка ещё полностью на месте) — пик — не полный 1 (глухая
  // чернота на миг читалась бы как баг/фриз), а 0.94
  const VEIL_HALF_PX = 900;
  ...
  // сцен, волной от rawScrollY: 0 на подходе → почти сплошной чёрный
  // ровно в момент передачи → снова 0, когда «Знакомство» уже само на
  // весь экран. Симметрично работает и при скролле вверх (см. Math.abs).
  // smoothstep вместо прямой линии — САМ подход/выход из черноты идёт
  // мягче (медленнее в начале и в конце, быстрее в середине), поэтому
  // самый первый и самый последний проценты затемнения незаметны глазу
  // ещё сильнее, чем просто широкое линейное окно само по себе
  if (servicesExitVeilEl) {
    const veilDist = Math.abs(rawScrollY - servicesBottom);
    const veilT = veilDist < VEIL_HALF_PX ? 1 - veilDist / VEIL_HALF_PX : 0;
    servicesExitVeilEl.style.opacity = smoothstep(veilT) * VEIL_PEAK;
  }
}
...
// (0.18 и 0.6) осознанно — у обоих есть настоящая кульминация (стрела
